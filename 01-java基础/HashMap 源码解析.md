...
threshold 是阈值，县暂且不管阈值是啥，我们看到它调用了 tableSizeFor 这个方法，直接去看方法里面做了啥。
...
由此可以得出规律：该算法的作用是把最高位 1 后面的数值全部变成 1，最后将计算出来的结果 + 1。
...
依次类推，可以说这里把阈值设置成与给定容量最接近的一个 2 的幂次方（包含等于）。
 ...
 * @param value the value to put // key 对应的 value
...
在 putVal 方法的第四行中，注意到它调用了 resize 方法。先去看 resize 源码
